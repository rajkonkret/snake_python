...
def snake_growth(d_x, x, y):
    ...
    if d_x != 0:
        snake_all.append(Snake_element(iter, snake_all[-1].x + snake_font_size * 0.7 * x_sign,
                                       snake_all[-1].y + snake_font_size * 0.7 * y_sign))
        iter += 1
        print(d_x)
...
def detect_colision(x, y):
    global score
    for apple in apple_all:
        if ((snake_all[0].x in range(int(apple.x), int(apple.x + 22))) and (
                snake_all[0].y in range(int(apple.y), int(apple.y + 21)))) or (
                ((snake_all[0].x + 21) in range(int(apple.x), int(apple.x + 22))) and (
                (snake_all[0].y + 21) in range(int(apple.y), int(apple.y + 21)))):
            print("collision")
            print(apple.id)
...
while True:
    ...
    for action in pygame.event.get():
        ...
        print("type: ", action.type is pygame.KEYDOWN)
        print("type: ", action.type is pygame.KEYUP)
        if action.type == pygame.KEYUP:
            print("action key" + str(action.key))
